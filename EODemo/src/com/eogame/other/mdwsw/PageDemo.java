...
//	//正式环境参数
	private String gameCode = "LUNA";		//游戏gameCode
	private String gameKey = "3RiqOzwAspzg";	//游戏gameKey
	private boolean isDebug = true;			//是否为测试模式。//true为测试环境 //false为正式环境
	private String gameName = "LUNAM";		//游戏名
	private String gameLanguage = "en-us";  //游戏语言
	...
	public void onClick(View v) {
		...
		case R.id.page_main_btn_pay:
			...
			roleInfo.setRoleId("51");			//角色id
			roleInfo.setRoleLevel(150);			//角色等级
			roleInfo.setRoleName("天下无敌");		//角色名
			roleInfo.setServerId("2");			//服务器id
			roleInfo.setServerName("旅行的开始");	//服务器名称
			
			EOPayInfo payInfo = new EOPayInfo();
			payInfo.setProductName("60台币测试");//商品名称
			payInfo.setProductId("com.other.test1");//商品id//android.test.purchased为假定购买成功商品android.test.canceled为假定购买取消商品android.test.item_unavailable为假定商品不存在
			payInfo.setExtInfo("");
			payInfo.setPrice(600);  //商品金额 单位分
		...
		case R.id.page_main_btn_share:
			//分享到LINE
			//mSdk.shareTextToLine(this,"分享内容测试");
			mSdk.shareImageToLine(this,"/storage/emulated/0/Pictures/Screenshots/Screenshot_20200716_080846_com.netease.mhxyhtb.jpg");
			//分享到Facebook
